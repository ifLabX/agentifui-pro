...
1. Migrating to Pydantic v2 patterns (26 deprecation warnings)
1. Fixing test assertions and HTTP status codes (17 test failures)
1. Removing dead code (unused environment variables)
...
1. host, port
1. database_url, database_pool_size, database_pool_max_overflow, database_pool_timeout, database_pool_recycle
1. health_check_timeout, database_health_check_timeout
1. log_level, log_format
1. secret_key, algorithm, access_token_expire_minutes (will be removed in T003)
1. cors_origins, cors_allow_credentials, cors_allow_methods, cors_allow_headers
1. environment
1. enable_docs, enable_redoc, use_uuidv7 (will be removed in T003)
...
1. `test_middleware_stack_startup`
1. `test_dependency_injection_startup`
...
1. `test_quickstart_database_unhealthy_scenario` - expect 503 not 500
1. `test_quickstart_database_connection_test` - expect 503 not 500
1. `test_quickstart_cors_configuration` - fix CORS assertion
1. `test_quickstart_async_client_scenario` - fix async test pattern
1. `test_quickstart_verification_scenarios` - update status codes
1. `test_quickstart_file_structure_validation` - verify file paths
1. `test_quickstart_logging_configuration` - fix log level check
