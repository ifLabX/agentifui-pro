...
1. **Parallel cleanup**: Phase 6 tasks must be sequential (directory removal order matters for error messages)
1. **Incomplete import updates**: Use grep verification (T014, T015) to catch missed imports
1. **Test failures ignored**: Any test failure blocks cleanup (T019 is critical gate)
