...
1. **Package discovery**: Automatic with `packages = find:` and `where = ["src"]`
1. **Import paths**: Absolute imports, no changes needed in source files
1. **pytest**: Add src/ to pythonpath, use importmode=importlib
1. **Alembic**: Update migrations/env.py to import from src.\* modules
1. **Ruff**: Add `src = ["src"]` to configuration
1. **Uvicorn**: Load as `src.main:app` instead of `main:app`
1. **Environment**: Preserve existing .env, create from example if missing
